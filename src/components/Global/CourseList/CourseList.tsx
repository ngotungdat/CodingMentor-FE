	...
	const fetchDataForFilterForm = async () => {
		try {
			const res = await Promise.all([
				...
				teacherApi.getAll({ pageIndex: 1, pageSize: 99999 })
			])
				.then(([branchRes, programRes, teacherRes]) => {
					const newOptionList = {
						branchList: [],
						programList: [],
						teacherList: []
					...
					teacherRes.status === 200 &&
						(newOptionList.teacherList = fmSelectArr(teacherRes.data.data, 'FullNameUnicode', 'UserInformationID'))
