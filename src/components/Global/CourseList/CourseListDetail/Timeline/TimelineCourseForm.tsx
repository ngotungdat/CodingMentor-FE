			...
			<Modal title="Thêm phản hồi" visible={isModalVisible} onCancel={() => setIsModalVisible(false)} footer={null}>
				<div>
					<Form layout="vertical" onFinish={form.handleSubmit(checkHandleSubmit)}>
						<div className="row">
							<div className="col-12 h-135">
								<TextAreaField form={form} name="Note" label="Phản hồi" placeholder="Nhập phản hồi" rows={5} />
							</div>
							<div className="col-12 h-135">
								<TextAreaField form={form} name="NoteStudent" label="Phản hồi về học viên" placeholder="Nhập phản hồi" rows={5} />
							</div>
							<div className="col-12">
								<button type="submit" className="btn btn-primary w-100" disabled={isLoading.type == 'ADD_TIMELINE' && isLoading.status}>
									Lưu
