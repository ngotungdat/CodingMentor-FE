...
const EditSelfCourse = (props) => {
	...
	const onCheckStudyTimeAvailable = async (params: ISCCheckSchedule) => {
		console.log('----------------------------------------------------------------')
		console.log('----onCheckStudyTimeAvailable')
	...
	useEffect(() => {
		if (Array.isArray(dataModalCalendar.scheduleList) && dataModalCalendar.scheduleList.length >= 1) {
			setIsLoading({ type: 'SCHEDULE_INVALID', status: false })
			dataModalCalendar.scheduleList.forEach((s) => {
				...
				if (isValid) {
					...
					setIsLoading({ type: 'SCHEDULE_INVALID', status: true })
				}
			})
		}
	...
	const createCalendarBlankList = async (eDate: string) => {
		...
		do {
			...
			if (!dayOffList.includes(Day) && Day !== now) {
				dates.push({ Day })
			}
					...
					<Card
						title={
							<>
								...
								<p style={{ fontSize: 13, color: '#bfbfcb' }}>Chỉ được xếp buổi học trước 24 tiếng, hủy buổi học trước 5 tiếng.</p>
							</>
						}
						extra={
							<>
								...
								<Schedule>
									<ScheduleList>
										{scheduleList.available.map((s, idx) => (
											<ScheduleSelfItem key={idx} scheduleObj={s} handleChangeStatusSchedule={onToggleSchedule} isEditView={true} />
										))}
									</ScheduleList>
								</Schedule>
							}
