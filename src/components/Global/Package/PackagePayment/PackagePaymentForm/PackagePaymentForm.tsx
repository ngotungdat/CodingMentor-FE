...
function PackagePaymentForm(props) {
	const { isLoading, updateObj, handleUpdatePackagePayment, approvalOptionList } = props
	...
	return (
		<>
			...
			<Modal centered title="Cập nhật trạng thái thanh toán" visible={isModalVisible} footer={null} onCancel={closeModal}>
				<div className="wrap-form">
					<Form layout="vertical" onFinish={form.handleSubmit(checkHandlePackagePayment)}>
						<div className="row">
							<div className="col-md-12 col-12">
								<SelectField form={form} name="Approval" label="Trạng thái" placeholder="Chọn trạng thái" optionList={approvalOptionList} />
							</div>
							{!!updateObj?.Note && (
								<div className="ml-3 mr-3 mb-2">
									<span className="bold">Ghi chú: </span> {updateObj?.Note}
								</div>
							)}
							<div className="col-md-12 col-12 mt-3 " style={{ textAlign: 'center' }}>
								<button type="submit" className="btn w-100 btn-primary" disabled={isLoading.type == 'ADD_DATA' && isLoading.status}>
									Cập nhật
