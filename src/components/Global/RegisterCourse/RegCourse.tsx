...
const RegCourse = React.memo((props: any) => {
	...
	const fetchList = () => {
		;(async () => {
			try {
				const _program = await programApi.getAll({ selectAll: true, isNotVideo: true })
	...
	const setBillCourseDetails = (value) => {
		if (value?.type == 1) {
			someThink.push({ BranchID: 1, CourseID: value?.value[value?.value.length - 1] })
		} else {
			someThink.push({ BranchID: 1, ProgramID: value?.value[value?.value.length - 1] })
		}
	}

	const handleChangeCourse = (value) => {
		setBillCourseDetails(value)
		let _totalStudents = []
		for (let i = 0; i < course.length; i++) {
			for (let j = 0; j < value?.value.length; j++) {
				if (course[i].ID == value?.value[j]) {
					_totalStudents.push({
	...
	const handleChangeProgram = (value) => {
		...
		for (let i = 0; i < program.length; i++) {
			for (let j = 0; j < value?.value.length; j++) {
				if (program[i].ID == value?.value[j]) {
					_price.push(program[i].Price)
				}
			}
		}
		function sumArray(arr) {
		...
		setTotalPriceProgram(sumArray(_price))
	}
	...
	return (
		<Card title="Đăng ký khóa học">
			...
			<Spin spinning={loadingCourse}>
				<div className="row">
					<div className="col-12">
						<Form.Item name="ProgramID" label="Chương trình mong muốn">
							<Select
								mode="multiple"
								className="style-input"
								showSearch
								optionFilterProp="children"
								onChange={(value: any) => {
									handleChangeProgram({ value: value, type: 1 })
								}}
							>
