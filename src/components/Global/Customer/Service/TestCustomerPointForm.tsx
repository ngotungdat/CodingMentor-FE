...
const TestCustomerPointForm = (props) => {
	...
	const { isLoading, rowID, _onSubmit, getIndex, index, rowData, listData, teacherList } = props
	...
	const onSubmit = (data: any) => {
		if (data.MaxTuitionOfStudent) {
			data.MaxTuitionOfStudent = parseFloat(data.MaxTuitionOfStudent.replace(/,/g, ''))
		}
		...
		if (isModalVisible) {
			if (rowData) {
				if (rowData.MaxTuitionOfStudent) {
					...
					rowData.MaxTuitionOfStudent = parseInt(money.replace(/\,/g, ''), 10).toLocaleString()
				}
	...
	return (
		<>
			...
			<Modal title="Bảng điểm test" visible={isModalVisible} onCancel={() => setIsModalVisible(false)} footer={null}>
				<div className="wrap-form">
					<Form layout="vertical" onFinish={form.handleSubmit(onSubmit)}>
						{/*  */}
						<div className="row">
							<div className="col-md-6 col-12">
								<InputTextField form={form} name="ListeningPoint" label="Listening" />
							</div>
							<div className="col-md-6 col-12">
								<InputTextField form={form} name="SpeakingPoint" label="Speaking" />
							</div>
						</div>
						{/*  */}
						<div className="row">
							<div className="col-md-6 col-12">
								<InputTextField form={form} name="ReadingPoint" label="Reading" />
							</div>
							<div className="col-md-6 col-12">
								<InputTextField form={form} name="WritingPoint" label="Writing" />
							</div>
						</div>
						<div className="row">
							<div className="col-md-6 col-12">
								<InputTextField form={form} name="VocabPoint" label="Vocabulary" />
							</div>
							<div className="col-md-6 col-12">
								<InputMoneyField form={form} name="MaxTuitionOfStudent" label="Học phí tối đa" />
							</div>
						</div>
						<div className="row">
							<div className="col-md-12 col-12">
								<SelectField form={form} name="TeacherID" label="Giáo viên" optionList={teacherList} />
							</div>
						</div>
						{/*  */}
						<div className="row">
