											...
											<>
												{detailResult.map((item, index) => {
													if (item.ExerciseGroupID !== 0) {
																	...
																	<div className="row">
																		...
																		{(userInformation?.RoleID == 1 || userInformation?.RoleID == 2) && (
																			<div className="col-md-1">
																				<div className="point-question text-right mt-4">{item.ExerciseTopic[0]?.Point || 0}</div>
																			</div>
																		)}
																	</div>
														...
														return (
															<div key={index}>
																<ListQuestion showScore={false} isMarked={isMarked} dataQuestion={item} listQuestionID={listQuestionID} />
															</div>
														)
													}
												})}
