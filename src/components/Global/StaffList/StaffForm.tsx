	...
	const getListRole = (e) => {
		if (e !== null) {
			...
			for (let i = 0; i < e.length; i++) {
				temp.push({ value: e[i]?.ID, title: e[i]?.name })
			}
			return temp
		}
	}
		...
		<>
			...
			>
				{statusAdd == 'add-staff' ? (
							...
							<div className="row">
								...
								<div className="col-md-6 col-12">
									<SelectField
										...
										name="RoleID"
										label="Vị trí"
										optionList={getListRole(useStaffRoles)}
										onChangeSelect={
					...
					<SalaryForm dataStaff={dataStaff} changeVisible={changeVisible} isLoading={isLoading} submitSalary={submitSalary} />
				)}
			</Modal>
		</>
	)
