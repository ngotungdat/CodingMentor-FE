...
;(function () {
	...
	e.AmSerialChart = e.Class({
					...
					if (l.mouseIsOver()) {
						...
						0 > a
							? d
								...
								: (0 < f && 0 < e && this.end >= l - 1 && (e = f = 0), (d = this.start + e), (g = this.end + f), this.zoomToIndexes(d, g))
							: d
							? ((l = this.endTime - this.startTime),
							  (d = this.startTime - e * g),
							  (g = this.endTime - f * g),
		...
		handleCursorZoomReal: function (a, b, c, d, e) {
			...
			isNaN(f) ||
				isNaN(l) ||
				((a = this.categoryAxis), a.parseDates && !a.equalSpacing ? this.zoomToDates(new Date(f), new Date(l)) : this.zoomToIndexes(f, l))
		},
					...
					if ((g = this.chartData[g])) {
						...
						if (this.prevCursorItem != g || k != this.prevMostCloseGraph) {
							...
							for (h = 0; h < d.length; h++) {
								...
								k && f != k
									? (f.showGraphBalloon(l, b.pointer, !1, b.graphBulletSize, b.graphBulletAlpha), f.balloon.hide(0))
									: b.valueBalloonsEnabled
									? ((f.balloon.showBullet = b.bulletsEnabled),
									  (f.balloon.bulletSize = b.bulletSize / 2),
									  a.hideBalloons ||
											(f.showGraphBalloon(l, b.pointer, !1, b.graphBulletSize, b.graphBulletAlpha),
											f.balloon.set && m.push({ balloon: f.balloon, y: f.balloon.pointToY })))
		...
		handleCursorPanning: function (a) {
			...
			if (b.zoomable && (0 < Math.abs(d) || 0 < Math.abs(h)))
				if (c.parseDates && !c.equalSpacing) {
					...
					(this.skipZoomed = !0), b.fire({ chart: this, target: b, type: 'zoomed', start: a, end: c }), this.zoom(a, c), (n = !0)
	...
	e.Cuboid = e.Class({
		...
		draw: function () {
			...
			!isNaN(t) && (0 < Math.abs(h) || 0 < Math.abs(f))
				? ((l = [B]),
				  (l = { fill: l, stroke: p, 'stroke-width': m, 'stroke-opacity': n, 'fill-opacity': k }),
				  K
								...
								? ((k +=
								  ...
								  (k =
										...
										(m +
											...
											g +
											',0,0')))
								: ((k += 'A' + I + ',' + J + ',0,0,0,' + d + ',' + (g - (g / 2) * (1 - t)) + 'L0,' + g),
								  (k += 'A' + G + ',' + H + ',0,0,1,0,0')),
						  (G = 90))
						: ((m = d / 2 - (d / 2) * t),
						  (k = 'M0,0 L' + m + ',' + g),
						  e.VML
								? ((k = 'M0,0 L' + m + ',' + g),
								  ...
								  (k +=
										...
										d +
										',0,0,0'))
								: ((k += 'A' + I + ',' + J + ',0,0,0,' + (d - (d / 2) * (1 - t)) + ',' + g + 'L' + d + ',0'),
								  (k += 'A' + G + ',' + H + ',0,0,1,0,0')),
						  (G = 180)),
				  (b = b.path(k).attr(l)),
				...
				: (b =
						0 === g
							...
							: e.polygon(b, [0, 0, d, d, 0], [0, g, g, 0, 0], l, k, m, p, n, u, !1, x))
			d = isNaN(t)
				? 0 > g
					? [w, M, y, C, F, D, A, z, L, b]
					: [z, L, y, C, F, D, w, M, A, b]
		...
		draw: function () {
			...
			if (e.ifArray(a)) {
				...
				if (this.parseDates && !this.equalSpacing)
					...
					if (
						((this.cellWidth = this.getStepWidth(f)), f < l && (l = f), (h += this.start), (this.stepWidth = this.getStepWidth(f)), 0 < l)
					)
						for (
							q = Math.floor(f / l),
				...
				if (this.widthField)
					for (b = this.start; b <= this.end; b++)
						(t = this.data[b]),
							...
							this.rotate
								? ((m = (this.height * d) / y / 2 + k), (k = (this.height * d) / y + k))
								: ((m = (this.width * d) / y / 2 + k), (k = (this.width * d) / y + k)),
							(t.x[this.id] = m)
				...
				for (b = 0; b < y; b++)
					if (
						...
						(w = c.dataDateFormat),
						k.toDate &&
							(!w || k.toDate instanceof Date || (k.toDate = k.toDate.toString() + ' |'),
							(k.toDate = e.getDate(k.toDate, w)),
							this.equalSpacing
								? ((q = c.getClosestIndex(this.data, 'time', k.toDate.getTime(), !1, 0, this.data.length - 1)),
						...
						k.date &&
							...
							this.equalSpacing
								? ((q = c.getClosestIndex(this.data, 'time', k.date.getTime(), !1, 0, this.data.length - 1)),
