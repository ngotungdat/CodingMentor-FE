	...
	const columns = [
...

		{
			render: () => {
				return (
					<>
						<Button style={{ marginLeft: 10 }} icon={<EyeOutlined />} type="primary" onClick={() => router.push('./course-map-detail')}>
							Detail
						</Button>
					</>
				)
				...
				<div className="col-12">
					...
					<div className="wrap-table">
						<Card
							...
							extra={<Search placeholder="Tìm kiếm" className="btn-search" size="large" />}
						>
							<PowerTable columns={columns} dataSource={data} />
						</Card>
					</div>
				</div>
				{/* <AddCourseMap visible={addCourseMap} onCancel={() => setAddCourseMap(false)} /> */}
			</div>
		</>
	)
}
