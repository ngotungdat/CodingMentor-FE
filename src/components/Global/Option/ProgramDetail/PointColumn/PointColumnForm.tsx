	...
	const schema = yup.object().shape({
		...
		Coefficient: yup
			...
			.when('Type', (type, schema) => {
				return type !== 1 ? schema : schema.min(1, 'Hệ số tối thiểu: 1').max(10, 'Hệ số tối đa: 10').required('Bạn không được để trống')
			})
	})
			...
			<Modal title={isUpdate ? 'Cập nhật cột điểm' : 'Tạo cột điểm'} visible={isModalVisible} onCancel={closeModal} footer={null}>
				<div>
					<Form layout="vertical" onFinish={form.handleSubmit(pointColumnSwitchFunc)}>
						...
						<div className="row">
							<div className="col-12">
								<InputTextField form={form} name="Coefficient" label="Hệ số" disabled={isDisabled} placeholder="Nhập hệ số" />
							</div>
						</div>
