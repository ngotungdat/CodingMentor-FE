...
export default function CustomerServiceExam() {
	...
	const columns = [
		...
		},
		{
			...
			render: (typeTest) => {
				return <>{typeTest == 'Thi Thử' ? <span className="tag blue">{typeTest}</span> : <span className="tag green">{typeTest}</span>}</>
			}
		},
		// {
		//   title: "Nhà cung cấp",
		...
		// },
		{
			...
			render: (Price) => {
				return <p className="font-weight-black">{Intl.NumberFormat('ja-JP').format(Price)}</p>
			}
		},
		{
			title: 'Ngày thi',
		...
		},
		{
			render: (record) => (
				<>
					...
					<Link
						href={{
							pathname: '/customer/service/service-customer-exam/student-detail/[slug]',
							query: { slug: 2 }
						}}
					>
