...
import { Drawer, Form, Modal, Spin, Tooltip } from 'antd'
...
const CreateQuestionForm = (props) => {
	...
	const [describeAnswer, setDescribeAnswer] = useState('')
	const [reset, setReset] = useState(false)

	const showDrawer = () => {
		if (!!questionData?.Type && questionData?.Type !== 0) {
			setVisible(true)
		} else {
	...
	return (
		<>
			...
			>
				...
				{/* {!isGroup?.status && (
					<div className="col-12 pr-4" style={{ fontWeight: '600', color: 'rgba(0,0,0,0.8)' }}>
						<div className="mb-3">Giải thích đáp án</div>
						<EditorSimple handleChange={(value: any) => setDescribeAnswer(value)} isReset={reset} />
					</div>
				)} */}
