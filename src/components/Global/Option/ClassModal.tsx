import { Form, Input, Modal, Select, Skeleton, Spin, Tooltip } from 'antd'
...
const ClassModal = React.memo((props: any) => {
	...
	const { isLoading, startShowModal, dataBranch, dataCourse, dataRoom, getRoom, getCourse } = props
	...
	const onChangeSelect = (name: any) => (value: any, option: any) => {
		name == 'BranchName' &&
			(getRoom(value), getCourse(value), setValue('RoomName', ''), setValue('ListCourseName', ''), setValueRoom(''), setValueCourse(''))
								...
								<Form.Item label="Code lớp">
									{isLoading.type == 'GET_WITH_ID' && isLoading.status ? (
										...
										<Input
											...
											onChange={(e) => setValue('ListClassCode', e.target.value)}
										/>
									)}
								</Form.Item>
							</div>
								...
								<Form.Item label="Tên lớp">
									{isLoading.type == 'GET_WITH_ID' && isLoading.status ? (
										...
										<Input
											...
											onChange={(e) => setValue('ListClassName', e.target.value)}
										/>
									)}
								</Form.Item>
							</div>
							...
							<div className="col-md-6 col-12">
								<Form.Item label="Trung tâm">
									{isLoading.type == 'GET_WITH_ID' && isLoading.status ? (
										...
										>
											{dataBranch?.length > 0 ? (
												dataBranch?.map((item) => <Option value={item.ID}>{item.BranchName}</Option>)
											) : (
							...
							<div className="col-md-6 col-12">
								<Form.Item label="Khóa học">
									{isLoading.type == 'GET_WITH_ID' && isLoading.status ? (
										...
										>
											{dataCourse?.length > 0 ? (
												dataCourse?.map((item) => <Option value={item.ListCourseID}>{item.ListCourseName}</Option>)
											) : (
												<Option value={'none'}>Không có dữ liệu</Option>
											)}
										</Select>
